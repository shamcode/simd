...
func Test_FetchAllAndTotal(t *testing.T) { //nolint:maintidx
	...
	testCases := []struct {
		...
		},
		{
			...
			Query: query.NewBuilder[*User]().
				Not().
				Where(query.Field(userStatus, where.EQ, StatusDisabled)).
				Sort(sort.Asc(userID)).
				Query(),
			ExpectedCount: 2,
			ExpectedIDs:   []int64{1, 4},
		},
		{
			...
			Query: query.NewBuilder[*User]().
				Where(query.Field(userScore, where.GE, 10)).
				Where(query.Field(userScore, where.LT, 20)).
				Sort(sort.Asc(userID)).
				Query(),
			ExpectedCount: 2,
		...
		},
		{
			...
			Query: query.NewBuilder[*User]().
				Where(query.Field(userName, where.EQ, "Fourth")).
				Where(query.Field(userStatus, where.EQ, StatusActive)).
				Query(),
			ExpectedCount: 1,
			ExpectedIDs:   []int64{4},
		...
		},
		{
			...
			Query: query.NewBuilder[*User]().
				Where(query.Field(userName, where.Like, "t")).
				Sort(sort.Asc(userID)).
				Query(),
			ExpectedCount: 2,
			ExpectedIDs:   []int64{1, 4},
		},
		{
			...
			Query: query.NewBuilder[*User]().
				Where(query.Field(userID, where.EQ, 1)).
				Or().
				Where(query.Field(userStatus, where.EQ, StatusDisabled)).
				Sort(sort.Asc(userID)).
				Query(),
		...
		},
		{
			...
			Query: query.NewBuilder[*User]().
				...
				Where(query.Field(userStatus, where.EQ, StatusDisabled)).
				Or().
				Where(query.FieldBool(userIsOnline, where.EQ, false)).
				CloseBracket().
				Sort(sort.Asc(userID)).
				Query(),
			ExpectedCount: 3,
		...
		},
		{
			...
			Query: query.NewBuilder[*User]().
				Where(query.Field(userID, where.EQ, 4)).
				Or().
				OpenBracket().
				Where(query.Field(userStatus, where.EQ, StatusDisabled)).
				Or().
		...
		},
		{
			...
			Query: query.NewBuilder[*User]().
				Where(query.Field(userID, where.InArray, 4, 2)).
				Sort(sort.Asc(userID)).
				Query(),
			ExpectedCount: 2,
		...
		},
		{
			...
			Query: query.NewBuilder[*User]().
				Where(query.FieldStringRegexp(userName, regexp.MustCompile("[tT]"))).
				Sort(sort.Asc(userID)).
				Query(),
			ExpectedCount: 3,
		...
		},
		{
			...
			Query: query.NewBuilder[*User]().
				Where(query.Field(userName, where.InArray, "Second", "Third")).
				Sort(sort.Asc(userID)).
				Query(),
			ExpectedCount: 2,
		...
		},
		{
			...
			Query: query.NewBuilder[*User]().
				...
				Where(query.Field(userID, where.EQ, 1)).
				CloseBracket().
				Where(query.Field(userID, where.InArray, 1, 2, 3)).
				Sort(sort.Asc(userID)).
				Query(),
			ExpectedCount: 1,
			ExpectedIDs:   []int64{1},
		...
		},
		{
			...
			Query: query.NewBuilder[*User]().
				Where(query.FieldSet(userTags, where.SetHas, TagConfirmed)).
				Sort(sort.Asc(userID)).
				Query(),
			ExpectedCount: 3,
			ExpectedIDs:   []int64{1, 2, 3},
		},
		{
			...
			Query: query.NewBuilder[*User]().
				Where(query.FieldSet(userTags, where.SetHas, TagFree)).
				Sort(sort.Asc(userID)).
				Query(),
			ExpectedCount: 1,
			ExpectedIDs:   []int64{3},
		},
		{
			...
			Query: query.NewBuilder[*User]().
				Where(query.FieldMap(userCounters, where.MapHasKey, CounterKeyUnreadMessages)).
				Sort(sort.Asc(userID)).
				Query(),
			ExpectedCount: 2,
			ExpectedIDs:   []int64{1, 2},
		},
		{
			...
			Query: query.NewBuilder[*User]().
				Where(query.FieldMap(userCounters, where.MapHasValue, HasCounterValueEqual(2))).
				Sort(sort.Asc(userID)).
				Query(),
			ExpectedCount: 1,
			ExpectedIDs:   []int64{2},
		},
		{
			...
			Query: query.NewBuilder[*User]().
				Where(query.FieldMap(userCounters, where.MapHasValue, HasCounterValueEqual(1))).
				Sort(sort.Asc(userID)).
				Query(),
			ExpectedCount: 2,
			ExpectedIDs:   []int64{1, 4},
